    ...
    while (true)
    {
        ...
        for (int i = 0; i < mask_contours.size(); i++)
        {
            ...
            if (centers.size() == 2 && abs(centers[0].y - centers[1].y) <= 50)
                if (centers.size() == 2 && abs(centers[0].y - centers[1].y) <= 50)
                {
                    int distance = (int)norm(centers[0] - centers[1]); 
                    int lineThickness1 = 30 - (distance / (500/12));
                    int lineThickness2 = 20 - (distance / (500/8));
                    int lineThickness3 = 6 - (distance / (500/4));
                    
                    if(lineThickness3 > 0){
                        line(video, centers[0], centers[1], Scalar(0, 0, 255), lineThickness1);
                        line(video, centers[0], centers[1], Scalar(255, 0, 0), lineThickness2);
                        line(video, centers[0], centers[1], Scalar(0, 255, 0), lineThickness3);
                    }
                }
