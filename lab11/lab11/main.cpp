...
int main()
{
    ...
    cv::Ptr<aruco::DetectorParameters> parameters = cv::makePtr<aruco::DetectorParameters>();
    ...
    Mat car_image = imread(R"(/Users/maciejdubowik/Downloads/car0.png)");
    Mat car_image2 = imread(R"(/Users/maciejdubowik/Downloads/car1.png)");
    Mat goal_image = imread(R"(/Users/maciejdubowik/Downloads/obstacle.png)");
    ...
    while (waitKey(10) != 27) {
        ...
        auto found2 = find(markerIds.begin(), markerIds.end(), 1);
        ...
        if (found2 != markerIds.end()) {
            ...
            auto found2 = markerCorners.at(foundIdx);
            
            cv::Vec3d rvecs, tvecs;
            cv::solvePnP(obj_points, found2, camera_matrix, dist_coeffs, rvecs, tvecs);
            cv::drawFrameAxes(detected, camera_matrix, dist_coeffs, rvecs, tvecs, 10.1);
        ...
        for (int i = 0; i < goals.size(); i++) {
            if (cv::norm(goals[i] - car_position) < (goal_image.cols / 2)) {
                points++;
                std::cout << "Gracz pierwszy ma :" << points << "punktów" << " | Gracz drugi ma : " << points2 << "punktów" << std::endl;
        ...
        Mat insetImage2(detected,
                        Rect(
                             car_position2.val[0] - (car_image2.cols / 2),
                             car_position2.val[1] - (car_image2.rows / 2),
                             car_image2.rows, car_image2.cols));
        car_image2.copyTo(insetImage2);
        for (auto goal : goals) {
            Mat insetImage2(detected,
                            Rect(
                                 goal.val[0] - (goal_image.cols / 2),
                                 goal.val[1] - (goal_image.rows / 2),
                                 goal_image.rows, goal_image.cols));
            goal_image.copyTo(insetImage2);
        }
        for (int i = 0; i < goals.size(); i++) {
            if (cv::norm(goals[i] - car_position2) < (goal_image.cols / 2)) {
                points2++;
                std::cout << "Gracz1 ma:" << points << " punktów" << " | Gracz2 ma: " << points2 << " punktów" << std::endl;
                goals.erase(goals.begin() + i);
                i--;
            }
        }
        
        if(points >=3 || points2 >=3){
            break;
        }
        imshow("markers", detected);
    }
    if(points > points2){
        cout << "Wygrywa Gracz1 " << points  << endl;
    } else{
        cout << "Wygrywa Gracz2" << points2 << endl;
    }
